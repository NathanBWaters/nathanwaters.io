...
function extend() {
	...
	_function: while (_again) {
		_len = objects = _key = to = from = _last = intermediary_result = _iterator = _isArray = _i = _iterator = _ref = key = undefined;
		_again = false;
		...
		if (objects.length > 2) {
			...
			_this = undefined;
			_arguments = [intermediary_result, _last];
			_again = true;
			continue _function;
		}
...
function alias_properties_with_camel_case(object) {
	for (var _iterator3 = _Object$keys(object).filter(function (key) {
		return !is_private_property(key);
	}), _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _getIterator(_iterator3);;) {
		var _ref3;
